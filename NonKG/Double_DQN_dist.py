...
class Double_DQN(object):
    def __init__(self, NGCF, optimizer, action_num, learning_rate, state_shape, mlp_layers, device, discount_factor,
                 replay_size, batch_size, epsilon_decay, alpha, max_timestamp, count_train, large_K,
                 base_line_number):  # 추후에 추가하기 alpha can be erased
    ...
    def get_reward_user_new(self, user_idx, action_idx, item_list_for_reward, action_list_for_reward, hop_result,
                            train_dict):
        ...
        if len(neg_item_candidate) == 0:  # ml-1m
            ...
            for i in pos_item:
                ...
                neg_list_candidate = np.random.choice(neg_item_candidate)
                neg_item_list.append(neg_list_candidate - self.NGCF.n_user)
                neg_action_list.append(
                    action_list_for_reward[np.where(item_list_for_reward == neg_list_candidate)[0][0]])
    ...
    def get_reward_item_new(self, item_idx, action_idx, user_list_for_reward, action_list_for_reward, hop_result):
        ...
        if len(neg_user_candidate) == 0:  # ml-1m
            ...
            for i in pos_user:
                ...
                neg_user_list.append(neg_list_candidate)
                neg_action_list.append(
                    action_list_for_reward[np.where(user_list_for_reward == neg_list_candidate)[0][0]])
        ...
        if len(real_pos) != 0:
            ...
            for i in real_pos:
                ...
                for j in range(4):
                    ...
                    total_score += torch.matmul(item_embed_action, pos_embed.T).item() - torch.matmul(item_embed_action,
                                                                                                      neg_embed.T).item()
        ...
        if len(real_pos) != 0:
            ...
            for i in real_pos:
                ...
                for j in range(4):
                    ...
                    total_score += torch.matmul(user_embed_action, pos_embed.T).item() - torch.matmul(user_embed_action,
                                                                                                      neg_embed.T).item()
                temp_reward /= total_score
                reward += temp_reward
            reward = reward / len(real_pos)
    ...
    def learn_single(self, item, user, baseline_train, i, count):
        done = True
        with torch.no_grad():
    ...
    def learn(self, train_dict, baseline_train):
        ...
        for i in range(self.epsilon_decay):
            ...
            count = 0
            item, user = self.random_pick(train_dict)  # need parameter check --HS
            item2, user2 = self.random_pick(train_dict)
            item3, user3 = self.random_pick(train_dict)
            item4, user4 = self.random_pick(train_dict)
            item5, user5 = self.random_pick(train_dict)  # need parameter check --HS
            item6, user6 = self.random_pick(train_dict)
            item7, user7 = self.random_pick(train_dict)
            item8, user8 = self.random_pick(train_dict)
            item9, user9 = self.random_pick(train_dict)  # need parameter check --HS
            item10, user10 = self.random_pick(train_dict)
            item11, user11 = self.random_pick(train_dict)
            item12, user12 = self.random_pick(train_dict)
            item13, user13 = self.random_pick(train_dict)
            item14, user14 = self.random_pick(train_dict)
            item15, user15 = self.random_pick(train_dict)  # need parameter check --HS
            item16, user16 = self.random_pick(train_dict)
            item17, user17 = self.random_pick(train_dict)
            item18, user18 = self.random_pick(train_dict)
            item19, user19 = self.random_pick(train_dict)  # need parameter check --HS
            item20, user20 = self.random_pick(train_dict)

            self.learn_single(item, user, baseline_train, i, count)
            self.learn_single(item2, user2, baseline_train, i, count)
            self.learn_single(item3, user3, baseline_train, i, count)
            self.learn_single(item4, user4, baseline_train, i, count)
            self.learn_single(item5, user5, baseline_train, i, count)
            self.learn_single(item6, user6, baseline_train, i, count)
            self.learn_single(item7, user7, baseline_train, i, count)
            self.learn_single(item8, user8, baseline_train, i, count)
            self.learn_single(item9, user9, baseline_train, i, count)
            self.learn_single(item10, user10, baseline_train, i, count)
            self.learn_single(item11, user11, baseline_train, i, count)
            self.learn_single(item12, user12, baseline_train, i, count)
            self.learn_single(item13, user13, baseline_train, i, count)
            self.learn_single(item14, user14, baseline_train, i, count)
            self.learn_single(item15, user15, baseline_train, i, count)
            self.learn_single(item16, user16, baseline_train, i, count)
            self.learn_single(item17, user17, baseline_train, i, count)
            self.learn_single(item18, user18, baseline_train, i, count)
            self.learn_single(item19, user19, baseline_train, i, count)
            self.learn_single(item20, user20, baseline_train, i, count)
    ...
    def train_KGAT(self):
        ...
        for j in range(10):  # 100 나중에 hyperparameter로 바꾸기6
            for i in self.NGCF_train:
                ...
                self.optimizer.zero_grad()
                u_g_embeddings, pos_i_g_embeddings, neg_i_g_embeddings = self.NGCF(i[0], i[1], i[2], i[3], i[4], i[5],
                                                                                   0)
    ...
    def train_item(self):
        ...
        target_batch = target_batch.to(torch.float32)
        state_batch = state_batch.to(self.device)
        state_batch = self.NGCF.embedding_dict['item_emb'][
    ...
    def make_reward(self, train_dict):  # 우선은 데이터 하나당 돌려서 g.ndata['reward'] 업데이트 하는 방향
        ...
        for i in range(len(self.state_user)):
            ...
            if i == len(self.state_user) - 1:
                ...
                transition_user = to_memory(self.state_user[i + self.index_dict_add],
                                            self.action_user[i + self.index_dict_add], reward,
                                            self.state_user[i + self.index_dict_add + 1],
                                            1)
                transition_item = to_memory(self.state_item[i + self.index_dict_add],
                                            self.action_item[i + self.index_dict_add], reward,
                                            self.state_item[i + self.index_dict_add + 1],
                                            1)
            self.memory_user.append(transition_user)
            self.memory_item.append(transition_item)
        # if bool_:
